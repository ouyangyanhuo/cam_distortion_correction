...
### 3. 访问界面
**启动后会自动打开浏览器！** 如果没有自动打开，可以：
- 复制终端显示的 `file://` 链接到浏览器中打开
- 或手动双击打开 `frontend/index.html`
...
| POST | `/api/update_distortion` | 更新畸变参数 |
...
1. **前后端分离**: 前后端职责清晰分离，后端仅提供API服务
2. **易于导航**: 快速找到相关文件
3. **简单部署**: 从根目录运行`python app.py`，自动打开浏览器
4. **模块化**: 可独立修改前端或后端
