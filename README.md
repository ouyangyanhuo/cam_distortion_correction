...
**启动后会自动打开浏览器！** 应用将：
- 后端 API 在 `http://127.0.0.1:5000` 上运行
- 自动在默认浏览器中打开前端界面
- 终端中显示前端界面的 `file://` 链接，可手动复制打开

如果浏览器没有自动打开，请复制终端显示的链接到浏览器中，或者手动双击 `frontend/index.html` 文件。

### 使用流程

1. **启动应用**：运行 `python app.py`，浏览器会自动打开前端界面
2. **准备标定板**：打印项目中的 [Charuco_A4.pdf](Charuco_A4.pdf) 或使用棋盘格标定板
3. **选择摄像头**：从下拉菜单中选择要标定的摄像头
4. **调整参数**：设置合适的分辨率、帧率和曝光参数
5. **捕获标定图像**：将标定板放置在不同角度和位置，点击"捕获标定图像"，建议至少捕获10张以上
6. **执行标定**：点击"执行标定"完成摄像头标定
7. **调整畸变参数**（可选）：手动微调畸变系数，实时预览效果
8. **预览效果**：标定完成后可实时查看畸变校正效果
9. **导出结果**：可生成C++代码或保存为YAML文件
...
### app.py
Flask应用主入口，提供API服务（纯后端，不服务静态文件）。启动时自动打开浏览器访问前端界面。
...
本项目采用前后端分离架构：
- **后端**：Flask 提供纯 API 服务（`app.py` 和 `backend/` 目录）
- **前端**：独立的 HTML/CSS/JS 文件（`frontend/` 目录），采用现代化黑金主题设计
